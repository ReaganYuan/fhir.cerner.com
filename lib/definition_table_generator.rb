...
def patch_definition_table(content_key, action, version)
  DefinitionTableGenerator.new(version, action).patch_definition_table(content_key)
end
...
class DefinitionTableGenerator
  ...
  def patch_definition_table(content_key)
    render_table(content_key, "#{@action}", 'patch_definition_table', 'patch')
  end
